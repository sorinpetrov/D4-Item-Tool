...
def capture_and_process(best_stats):
    ...
    if equip_position:
        ...
        # Detect item type using the full extracted text
        cleaned_extracted_text = re.sub(r'[^a-zA-Z\s]', '', extracted_text).lower()
        ...
        # Check stats using bounding boxes text
            ...
            for text, bbox in bounding_boxes:
                for priority, stats in priority_match.items():
                    for stat in stats:
                        ...
                        if fuzz.ratio(text_clean, stat_clean) > 80:
                            adjusted_bbox = (
                                bbox[0] + region['left'], 
                                bbox[1] + region['top'], 
                                bbox[2], 
